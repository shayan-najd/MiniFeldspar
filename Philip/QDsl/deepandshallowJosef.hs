...
data Option a = O { unO :: forall b . Undef b => (a -> (FunC Bool, b)) -> (FunC Bool, b) }

instance Undef a => Syntactic (Option a) where
  type Internal (Option a) = (Bool, Internal a)
  fromFunC t = O (\k -> (Fst t, ifC (Fst t) (snd (k (fromFunC (Snd t)))) undef))
  toFunC (O f) = toFunC (f (\a -> (true,a)))
...
none     =  O (\k -> (false, undef))

option   :: (Undef a, Undef b) => b -> (a -> b) -> Option a -> b
option noneCase someCase (O f) =
  case f (\a -> (true,a)) of
    (isSome, fromSome) -> ifC isSome (someCase fromSome) noneCase
