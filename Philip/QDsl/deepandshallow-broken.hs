...
data Option a where
  Return :: a -> Option a
  Bind   :: Opt a -> (a -> Option b) -> Option b
...
instance Monad Option where
  return a          =  Return a
  Return a >>= k    =  k a
  (Bind o h) >>= k  =  Bind o (\x -> h x >>= k)

lift :: Opt a -> Option a
lift o  =  Bind o Return

lower :: Syntactic a => Option a -> Opt a
lower (Return a)  =  Opt true a
lower (Bind o k)  =  Opt (mi ? (ni, false)) nf
  where
  Opt mi mf  =  o
  Opt ni nf  =  lower (k mf)
