module Examples.Feldspar.CRC.Feldspar where
...
tstInp :: [MP.Integer]
tstInp = MP.fmap (\ c -> MP.fromIntegral (ord c)) "test"       
...
updCrc cc ch =
    let c      = xor cc 0xFFFFFFFF
        newval = xor 
                 (tblVec !! ((xor c ch) .&. 0xff))
                 (c .>>. 8)
    in  xor newval 0xFFFFFFFF
