...
category $V$, type of each subrexpression inside the expression is
either in the set of subformulae of the type of the expression, or in
the set of subformulae of types in the environment:

 if $\Gamma \vdash \typing{V}{A}$, then for every subexpression $V_i$
 in $V$, such that $\Delta \vdash \typing{V_i}{A_i}$, we have $A_i \in
...
\item[Case ($\expprd{V}{W}$):]\ \\ 
  ...
  every $B_i$ (type of the subexpression $W_i$ in $W$), that $B_i \in
